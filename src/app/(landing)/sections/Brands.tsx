  ...
  return (
    <section id="brands" className="bg-ivory py-20 md:py-28">
      <div className="mx-auto max-w-[1320px] px-4 sm:px-6 md:px-12 space-y-12">
              ...
              <button
                ...
                className={`rounded-full border px-4 py-2 text-xs font-semibold transition-all duration-200 cursor-pointer ${
                  activeFilter === filter
                    ...
                    : "border-ocean/8 bg-white text-ocean-deeper/55 hover:border-ocean/20 hover:text-ocean"
                }`}
              >
                {filter}
              </button>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {featuredBrand && (
            <motion.article
              ...
              className="group relative overflow-hidden rounded-2xl border border-ocean/8 bg-white shadow-sm sm:col-span-2 lg:row-span-2 flex flex-col justify-between aspect-[1.2/1] lg:aspect-auto"
            >
              ...
              <div className="relative z-10 p-6 md:p-8 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/95 text-sm font-bold text-ocean-deeper shadow-sm">
                    {featuredBrand.logo}
                  </div>
                  <div>
                ...
                <p className="text-sm leading-[1.75] text-white/70 font-manrope max-w-md">
                  {featuredBrand.description}
                </p>
                <Link
                  href={`/brands/${featuredBrand.slug}`}
                  className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-5 py-2.5 text-xs font-bold text-white backdrop-blur-md transition-all duration-200 hover:bg-white/20 hover:-translate-y-[1px]"
                >
          ...
          {secondaryBrands.slice(0, 4).map((brand, index) => (
            <motion.article
              ...
              className="group flex flex-col justify-between rounded-2xl border border-ocean/8 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-[2px] hover:border-ocean/15 hover:shadow-md aspect-[1.15/1]"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  <span className="text-[9px] font-bold uppercase tracking-[0.22em] text-ocean/45 font-manrope">
                    {brand.category}
                  </span>
                  <h3 className="font-clash text-lg font-bold text-ocean-deeper leading-tight group-hover:text-ocean transition-colors">
                    {brand.name}
                  </h3>
                </div>
              ...
              <div className="my-4 flex flex-wrap gap-1.5">
                {brand.products.slice(0, 3).map((prod) => (
                  <span key={prod} className="rounded-full bg-ivory border border-black/5 px-2.5 py-1 text-[10px] font-medium text-ocean/60 font-manrope">
                    {prod}
                  </span>
                ))}
              </div>
              ...
              <div className="border-t border-black/5 pt-4 flex items-center justify-between text-xs">
                <span className="text-[11px] text-ocean/40 font-manrope">Genuine Stock</span>
                <Link
                  href={`/brands/${brand.slug}`}
                  className="inline-flex items-center gap-1 font-bold text-ocean-deeper hover:text-ocean transition-colors group"
                >
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-ocean/8 bg-white px-6 py-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-ocean/6 text-ocean shrink-0">
              <Sparkles className="h-5 w-5" />
            </div>
            <div>
              <p className="text-sm font-bold text-ocean-deeper">Direct Brand Channels</p>
              <p className="text-xs text-ocean/50 font-manrope">Galaxy Hub is an authorized partner supplying genuine warranties and authentic boxes across Rwanda.</p>
            </div>
          </div>
          <Link
            href="/brands"
