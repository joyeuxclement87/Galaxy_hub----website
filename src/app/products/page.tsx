        ...
        <div className="mx-auto max-w-[1320px] px-6 md:px-12">
          <section className="grid grid-cols-1 lg:grid-cols-8 gap-8 items-center mb-8">
            <div className="lg:col-span-6">
              ...
              <h1 className="font-clash text-3xl md:text-4xl font-bold text-ocean-deeper mt-3">
                {q ? `Results for "${q}"` : "Find Your Next Device"}
              </h1>
              <p className="mt-3 max-w-2xl text-sm text-ocean/55">
            ...
            <div className="lg:col-span-2 text-right">
              <div className="text-sm text-ocean/50">{result.total.toLocaleString()}+ Products</div>
            </div>
          </section>

          <nav className="sticky top-20 z-40 bg-ivory/80 py-3 backdrop-blur-sm">
            <div className="mx-auto max-w-[1320px] px-6 md:px-12">
              <div className="flex items-center gap-3 justify-between bg-transparent">
                <form method="get" action="/products" className="flex-1 pr-4">
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ocean/25" />
                    <input name="q" defaultValue={q} placeholder="Search Products..." className="w-full rounded-xl border border-ocean/8 pl-10 pr-4 py-3 text-sm bg-white" />
                  </div>
                </form>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <aside className="hidden lg:block lg:w-[280px] shrink-0">
                <div className="space-y-6">
                  {q && (
                    <div>
                      <h3 className="text-sm font-semibold text-ocean-deeper">Search</h3>
                      <p className="mt-1 text-xs text-ocean/50">Showing results for &ldquo;{q}&rdquo;</p>
                    </div>
                  )}

                  <div>
                    <h3 className="text-sm font-semibold text-ocean-deeper">Categories</h3>
                    <ul className="mt-3 space-y-2 text-sm text-ocean/60">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-ocean-deeper">Brands</h3>
                    <ul className="mt-3 space-y-2 text-sm text-ocean/60">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-ocean-deeper">Availability</h3>
                    <ul className="mt-3 space-y-2 text-sm text-ocean/60">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-ocean-deeper">Featured</h3>
                    <ul className="mt-3 space-y-2 text-sm text-ocean/60">
              ...
              <div className="flex-1">
                <div className="mb-6">
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-ocean/60">
                      Showing <span className="font-semibold text-ocean-deeper">{result.total > 0 ? start + 1 : 0}</span>
                      {" — "}
                      <span className="font-semibold text-ocean-deeper">{Math.min(start + 16, result.total)}</span>
                      {" of "}
                      <span className="font-semibold text-ocean-deeper">{result.total}</span>
                    </div>
                    <div className="text-sm text-ocean/50">Page {page} of {totalPages}</div>
                  </div>
                </div>

                {result.products.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-24 text-center">
                    <SlidersHorizontal className="mb-4 h-12 w-12 text-ocean/15" />
                    <h3 className="font-clash text-lg font-bold text-ocean-deeper">No products found</h3>
                    <p className="mt-2 text-sm text-ocean/50">Try adjusting your search or filters.</p>
                ...
                {totalPages > 1 && (
                  <div className="mt-12 flex items-center justify-center gap-3">
                    <Link href={buildHref({ ...params, page: Math.max(1, page - 1) })}
                      className={`px-4 py-2 rounded-xl border border-ocean/8 text-sm transition-colors ${page === 1 ? "opacity-40 pointer-events-none" : "hover:bg-white"}`}>
                      Previous
                    </Link>
                    {Array.from({ length: Math.min(totalPages, 8) }).map((_, i) => {
                      ...
                      return (
                        <Link key={i} href={buildHref({ ...params, page: pageNum })}
                          className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${pageNum === page ? "bg-ocean text-white shadow-sm" : "border border-ocean/8 hover:bg-white"}`}>
                          {pageNum}
                        </Link>
                      );
                    })}
                    ...
                    <Link href={buildHref({ ...params, page: Math.min(totalPages, page + 1) })}
                      className={`px-4 py-2 rounded-xl border border-ocean/8 text-sm transition-colors ${page === totalPages ? "opacity-40 pointer-events-none" : "hover:bg-white"}`}>
                      Next
                    </Link>
                  </div>
                )}
